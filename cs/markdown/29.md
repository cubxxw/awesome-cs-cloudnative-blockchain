...
API_KEY=661a2d8409a1b3e5ae62094dca33f10d
```
...
刮刀是基于[废料](https://scrapy.org/)的蟒蛇工具。我们确实建议使用[管道安装](https://github.com/pypa/pipenv)python环境。

+ [克隆抓取工具存储库](https://github.com/algolia/docsearch-scraper).
+ [安装管网](https://pipenv.readthedocs.io/en/latest/install/#installing-pipenv)
+ `pipenv install`
+ `pipenv shell`

如果计划使用浏览器仿真[（`js_render`设置为 true），](https://docsearch.algolia.com/docs/legacy/config-file)则需要执行此额外步骤。如果不这样做，则可以关闭此步骤。
