...
**我们在前面学习镜像的时候，不含有`vim/ifconfig`等命令，所以我们需要重新安装那些命令**
...
+ `dockerfile` 是软件原材料（写需要的东西）
+ `docker` 镜像是软件的交付品
+ `docker` 容器则可以认为是镜像的运行态，也即依照镜像运行的容器实例

![docker-graphic](https://sm.cubxxw.com/smJqBckovlsptOPSE.jpg?xxw@cubxxw.com)
