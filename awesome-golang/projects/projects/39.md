...
![图片](http://sm.cubxxw.com/sm202303062239320.png)

在编写 Kubernetes 资源定义文件的过程中，如果因为YAML格式文件中的配置项缩进太深，导致不容易判断配置项的层级，那么，你就可以将其转换成JSON格式，通过JSON格式来判断配置型的层级。
...
Kubernetes 中有很多内置的资源，常用的资源有 Deployment、StatefulSet、ConfigMap、Service、Secret、Nodes、Pods、Events、Jobs、DaemonSets等。除此之外，Kubernetes还有其他一些资源。如果你觉得Kubernetes内置的资源满足不了需求，还可以自定义资源。
...
> ⚠️ **注意：这个项目[不再维护](https://github.com/instrumenta/kubeval/issues/268#issuecomment-902128481)，一个很好的替代品是[kubeconform](https://github.com/yannh/kubeconform)**
...
❯ echo "======> ERROR END" >> iam-authz-server.yaml
❯ ./kubeval iam-authz-server.yaml
ERR  - Failed to decode YAML from iam-authz-server.yaml: error unmarshaling JSON: while decoding JSON: json: cannot unmarshal string into Go value of type map[string]interface {}
```
